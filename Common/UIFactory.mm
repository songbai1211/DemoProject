           ...
           target:(id)target
{
    if (button != nil) {
        ...
        if ((target != nil) && (selector != nil)) {
            [button setTarget:target action:selector forControlEvents:UIControlEventTouchUpInside];
        }
...
+ (void)showAlert:(NSString *)message
{
    UIAlertView *view =  [[UIAlertView alloc] initWithTitle:nil
                                                    message:message
                                                   delegate:nil
                                          cancelButtonTitle:[UIFactory localized:@"Alert_Ok"]
...
+ (void)showConfirm:(NSString *)message tag:(NSUInteger)tag delegate:(id)delegate
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@""
                                                        message:message
                                                       delegate:delegate
                                              cancelButtonTitle:[UIFactory localized:@"Alert_Cancel"]
                                              otherButtonTitles:[UIFactory localized:@"Alert_Ok"], nil];
    alertView.tag = tag;
    [alertView show];
}
